...
from keras.layers import LSTM, Flatten, AveragePooling1D
...
# temporal_lstm_consensus.py [train|retrain|test]{+cross} {batch} {classes} {epochs} {sample rate} {sequence length} {old epochs} {cross index}
if len(sys.argv) < 6:
    print 'Missing agrument'
    print r'Ex: temporal_lstm_consensus.py train {batch} {classes} {epochs} {sample rate} {sequence length}'
    sys.exit()
...
if process[0] == 'train':
    ...
    if len(sys.argv) < 7:
        print 'Missing agrument'
        print r'Ex: temporal_lstm_consensus.py retrain {batch} {classes} {epochs} {sample rate} {sequence length} {old epochs}'
        sys.exit()
    train = True
    retrain = True
...
result_model.compile(loss='categorical_crossentropy',
              optimizer=optimizers.SGD(lr=5e-5, decay=1e-6, momentum=0.9, nesterov=False),
              metrics=['accuracy'])

if train:
    if retrain:
        result_model.load_weights('weights/temporal_lstmconsensus{}_{}e_cr{}.h5'.format(opt_size,old_epochs,cross_index))
    ...
    for e in range(epochs):
        ...
        result_model.save_weights('weights/temporal_lstmconsensus{}_{}e_cr{}.h5'.format(opt_size,old_epochs+1+e,cross_index))

        with open('histories/temporal_lstmconsensus{}_{}_{}e_cr{}'.format(opt_size, old_epochs, epochs,cross_index), 'wb') as file_pi:
            pickle.dump(histories, file_pi)

else:
    result_model.load_weights('weights/temporal_lstmconsensus{}_{}e_cr{}.h5'.format(opt_size,epochs,cross_index))
    ...
    with open('results/temporal-lstmconsensus-{}-cr{}.txt'.format(opt_size,cross_index), 'w+') as fw1:
        fw1.write(classification_report(Y_test, y_classes, digits=6))
        fw1.write('\nRun time: ' + str(run_time))

    print 'Confusion matrix'
    print(confusion_matrix(Y_test, y_classes))
    with open('results/temporal-lstmconsensus-{}-cf-cr{}.txt'.format(opt_size,cross_index),'wb') as fw3:
        pickle.dump(confusion_matrix(Y_test, y_classes),fw3)

    with open('results/temporal-lstmconsensus-{}-cr{}.pickle'.format(opt_size,cross_index),'wb') as fw3:
